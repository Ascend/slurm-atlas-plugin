...
const char	*plugin_type		= "npu/generic";
...
dsmiReturn_t dsmiDeviceGetName(uint32_t device, char *name, unsigned int length)
{
	...
	strcpy(name, info.chip_type);
    strcat(name, " ");
    strcat(name, info.chip_name);
    strcat(name, " ");
    strcat(name, info.chip_ver);
			    ...
			    unsigned int *mem_freq_num, bool *verbose_flag)
{
	...
	def_freq = slurm_get_npu_freq_def();
				...
				unsigned int *gfx_freqs)
{
	...
	if (ret != 0) {
		error("%s: Failed to get supported graphics frequencies for the"
		      " NPU at mem frequency %u.", __func__);
		return false;
	}
...
static unsigned int _dsmi_get_gfx_freq(uint32_t device)
{
	...
	debug3("dsmi_get_device_frequency(AI core) took %ld microseconds",
	        DELTA_TIMER);
    if(ret != 0 ){
    	error("%s: Failed to get the NPU %s frequency: %s", __func__);
    	return 0;
    }
    return freq;
}
 ...
 */
static void _dsmi_get_device_count(unsigned int *device_count)
{
	*device_count = 0;
	int ret;
...
static List _get_system_npu_list_dsmi(node_config_load_t *node_config)
{
	...
	_dsmi_get_device_count(&device_count);
...
extern int npu_p_reconfig(void)
{
	if (slurm_get_debug_flags() & DEBUG_FLAG_GRES)
		log_lvl = LOG_LEVEL_INFO;
	else
...
extern List npu_p_get_system_npu_list(node_config_load_t *node_config)
{
