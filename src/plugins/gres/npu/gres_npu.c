/*****************************************************************************\
 *  gres_npu.c - Support NPUs as a generic resources.
 *****************************************************************************
 *
...
extern void step_hardware_init(bitstr_t *usable_npus, char *tres_freq)
{
	npu_g_step_hardware_init(usable_npus, tres_freq);
}

extern void step_hardware_fini(void)
{
	npu_g_step_hardware_fini();
}
		     ...
		     bool reset, bool is_job)
{
	...
	if (local_list) {
		env_array_overwrite(
			env_ptr, "ASCEND_VISIBLE_DEVICES", local_list);
		env_array_overwrite(
			env_ptr, "NPU_DEVICE_ORDINAL", local_list);
		xfree(local_list);
		*already_seen = true;
	}
}
...
/*
 * Sort gres/npu records by descending length of type_name. If length is equal,
...
/* Sort gres/npu records by "File" value */
static int _sort_npu_by_file(void *x, void *y, bool asc)
{
	gres_slurmd_conf_t *gres_record1 = *(gres_slurmd_conf_t **) x;
	gres_slurmd_conf_t *gres_record2 = *(gres_slurmd_conf_t **) y;
	int val1, val2;
...
/* Sort gres/npu records by "File" value in ascending order (nulls first) */
static int _sort_npu_by_file_desc(void *x, void *y)
{
	return _sort_npu_by_file(x, y, false);
}

/* Sort gres/npu records by "File" value in descending order */
static int _sort_npu_by_file_asc(void *x, void *y)
{
	return _sort_npu_by_file(x, y, true);
}

/*
 * Takes the merged [slurm|gres].conf records in gres_list_conf and the NPU
 * devices detected on the node in gres_list_system and returns a final merged
 * list in gres_list_conf.
 *
 * If a conf NPU corresponds to a system NPU, CPUs and Links are checked to see
 ...
 * gres_list_system - (in) The npu devices detected by the system. Each record
 ...
 *
 * A conf NPU and system NPU will correspond if the following fields are equal:
 * 	*type
 * 	*file
 */
static void _normalize_gres_conf(List gres_list_conf, List gres_list_system)
{
	...
	while ((gres_record = list_next(itr))) {
		...
		// Just move this GRES record if it's not a NPU GRES
		if (xstrcasecmp(gres_record->name, "npu")) {
			debug2("%s: preserving original `%s` GRES record",
			       __func__, gres_record->name);
			add_gres_to_list(gres_list_non_npu,
					 gres_record->name,
					 gres_record->count,
					 gres_record->cpu_cnt,
					 gres_record->cpus,
	...
	list_sort(gres_list_conf_single, _sort_npu_by_file_desc);
	/* Sort system devices in the same way for convenience */
	list_sort(gres_list_system, _sort_npu_by_file_desc);
	...
	while ((gres_record = list_next(itr))) {
		...
		if (sys_record) {
			...
			list_remove(itr2);
			list_append(gres_list_npu, sys_record);
			continue;
		}
		...
		if (gres_record->file) {
			...
			list_remove(itr);
			list_append(gres_list_npu, gres_record);
		} else {
			/*
	...
	if (gres_list_system && list_count(gres_list_system)) {
		info("WARNING: The following autodetected NPUs are being ignored:");
	...
	if (gres_list_npu && list_count(gres_list_npu)) {
		list_sort(gres_list_npu, _sort_npu_by_file_asc);
...
/*
 * Parses fake_npus_file for fake NPU devices and adds them to gres_list_system
 *
 * The file format is: <type>|<sys_cpu_count>|<cpu_range>|<links>|<device_file>
 *
 * Each line represents a single NPU device. Therefore, <device_file> can't
 ...
 */
static void _add_fake_npus_from_file(List gres_list_system,
				     char *fake_npus_file)
{
	char buffer[256];
	int line_number = 0;
	FILE *f = fopen(fake_npus_file, "r");
	if (f == NULL) {
		error("Unable to read \"%s\": %m", fake_npus_file);
		return;
	}
	...
	while (fgets(buffer, 256, f)) {
		...
		while (tok) {
			...
			case 2:
				if (tok[0] == '~')
					...
					cpu_range = npu_g_test_cpu_conv(tok);
				else
		...
		if (i != 5)
			error("Line #%d in fake_npus.conf failed to parse!"
...
/*
 * Creates and returns a list of system NPUs if fake_npus.conf exists
 * NPU system info will be artificially set to whatever fake_npus.conf specifies
 * If fake_npus.conf does not exist, or an error occurs, returns NULL
 * Caller is responsible for freeing the list if not NULL.
 */
static List _get_system_npu_list_fake(void)
{
	List gres_list_system = NULL;
	struct stat config_stat;
	char *fake_npus_file = NULL;

	/*
	 * Only add "fake" data if fake_npus.conf exists
	 * If a file exists, read in from a file. Generate hard-coded test data
	 */
	fake_npus_file = get_extra_conf_path("fake_npus.conf");
	if (stat(fake_npus_file, &config_stat) >= 0) {
		info("Adding fake system NPU data from %s", fake_npus_file);
		gres_list_system = list_create(destroy_gres_slurmd_conf);
		_add_fake_npus_from_file(gres_list_system, fake_npus_file);
	}
	xfree(fake_npus_file);
			    ...
			    node_config_load_t *node_config)
{
	...
	gres_list_system = _get_system_npu_list_fake();
	// Only query real system devices if there is no fake override
	if (!gres_list_system)
		gres_list_system = npu_g_get_system_npu_list(node_config);
	...
	if (gres_list_system) {
		if (list_is_empty(gres_list_system))
			log_var(log_lvl,
				"There were 0 NPUs detected on the system");
		log_var(log_lvl,
			"%s: Normalizing gres.conf with system NPUs",
			plugin_name);
			   ...
			   gres_epilog_info_t *epilog_info, int node_inx)
{
	...
	if (dev_num_str) {
		xstrfmtcat((*epilog_env_ptr)[env_inx++],
			   "ASCEND_VISIBLE_DEVICES=%s", dev_num_str);
		xstrfmtcat((*epilog_env_ptr)[env_inx++],
			   "NPU_DEVICE_ORDINAL=%s", dev_num_str);
		xfree(dev_num_str);
	}
