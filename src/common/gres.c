...
static bool have_gpu = false, have_mps = false, have_npu = false;
static uint32_t gpu_plugin_id = NO_VAL, mps_plugin_id = NO_VAL, npu_plugin_id = NO_VAL;
...
extern int gres_plugin_init(void)
{
	...
	while (one_name) {
		...
		if (!xstrcmp(one_name, "mps")) {
			...
			have_npu = true;
			npu_plugin_id = gres_plugin_build_id("npu");
		}
		if (!skip_name) {
					...
					void *xcpuinfo_mac_to_abs)
{
	...
	if (stat(gres_conf_file, &config_stat) < 0) {
		...
		if (s_p_get_string(&autodetect_string, "Autodetect", tbl)) {
			...
			// NPU plugin: add to detect NPU equipment
			if (xstrcasestr(autodetect_string, "npu"))
				autodetect_types |= GRES_AUTODETECT_NPU;
					    ...
					    char **reason_down)
{
	int i, rc, rc2;
	ListIterator gres_iter;
	gres_state_t *gres_ptr, *gres_gpu_ptr = NULL, *gres_mps_ptr = NULL, *gres_npu_ptr = NULL;
	...
	for (i = 0; i < gres_context_cnt; i++) {
		...
		else if (gres_ptr->plugin_id == npu_plugin_id)
			gres_npu_ptr = gres_ptr;
	}
					  ...
					  List *gres_list)
{
	...
	bool have_gres_gpu = false, have_gres_mps = false,have_gres_npu = false;
	...
	while ((gres_state = (gres_state_t *) list_next(iter))) {
		...
		if (!have_gres_gpu && !xstrcmp(job_gres_data->gres_name, "gpu"))
			have_gres_gpu = true;
		if (!have_gres_npu && !xstrcmp(job_gres_data->gres_name, "npu"))
			have_gres_npu = true;
	...
	while ((job_gres_ptr = (gres_state_t *) list_next(job_gres_iter))) {
		...
		if (core_bitmap && (bit_set_count(core_bitmap) == 0)) {
			...
			if (!use_total_gres && have_gpu && have_mps) {
				...
				if (job_gres_ptr->plugin_id == npu_plugin_id)
					alt_plugin_id = npu_plugin_id;
			}
			if (alt_plugin_id) {
				node_gres_iter =
 ...
 * OUT avail_gres     - Count of available gres on this node
 * OUT near_gres      - Count of gres available on sockets with available CPUs
 ...
 * IN cpu_per_gres - value to set as default
 * IN mem_per_gres - value to set as default
 */
extern void gres_plugin_job_set_defs(List job_gres_list, char *gres_name,
				     uint64_t cpu_per_gres,
				     uint64_t mem_per_gres)
{
	uint32_t plugin_id;
	ListIterator gres_iter;
	...
	while ((gres_ptr = (gres_state_t *) list_next(gres_iter))) {
		...
		if (!job_gres_data)
			continue;
		job_gres_data->def_cpus_per_gres = cpu_per_gres;
		job_gres_data->def_mem_per_gres = mem_per_gres;
	}
	list_iterator_destroy(gres_iter);
}
				     ...
				     int local_proc_id)
{
	...
	bool bind_npu = accel_bind_type & ACCEL_BIND_CLOSEST_NPU;
...
static bool _sharing_gres(uint32_t plugin_id)
{
	if (plugin_id == gpu_plugin_id)
		return true;
	else if (plugin_id == npu_plugin_id)
		return true;
	return false;
}
			     ...
			     char *type, char *links)
{
	 ...
	 */
	gres_record = list_next(itr);
	if (gres_record && (gres_record->count == 0))
		use_empty_first_record = true;
	else
		gres_record = xmalloc(sizeof(gres_slurmd_conf_t));
	gres_record->cpu_cnt = cpu_cnt;
	if (cpu_aff_mac_bitstr)
		gres_record->cpus_bitmap = bit_copy(cpu_aff_mac_bitstr);
	if (device_file)
		gres_record->config_flags |= GRES_CONF_HAS_FILE;
	if (type)
		gres_record->config_flags |= GRES_CONF_HAS_TYPE;
	gres_record->cpus = xstrdup(cpu_aff_abs_range);
	gres_record->type_name = xstrdup(type);
	gres_record->name = xstrdup(name);
	gres_record->file = xstrdup(device_file);
	gres_record->links = xstrdup(links);
	gres_record->count = device_cnt;
	gres_record->plugin_id = gres_plugin_build_id(name);
	if (!use_empty_first_record)
		list_append(gres_list, gres_record);
	list_iterator_destroy(itr);
}
