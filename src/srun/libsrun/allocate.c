...
static job_desc_msg_t *_job_desc_msg_create_from_opts(slurm_opt_t *opt_local)
{
	...
	if (opt_local->cpus_per_npu)
		xstrfmtcat(j->cpus_per_tres, "npu:%d", opt_local->cpus_per_npu);
	...
	xfmt_tres(&j->tres_per_job,    "npu", opt_local->npus);
	xfmt_tres(&j->tres_per_node,   "npu", opt_local->npus_per_node);
	...
	xfmt_tres(&j->tres_per_socket, "npu", opt_local->npus_per_socket);
	xfmt_tres(&j->tres_per_task,   "npu", opt_local->npus_per_task);
