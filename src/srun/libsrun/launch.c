					 ...
					 slurm_opt_t *opt_local)
{
	...
	if (opt_local->cpus_per_npu) {
		xstrfmtcat(job->ctx_params.cpus_per_tres, "npu:%d",
	...
	xfmt_tres(&job->ctx_params.tres_per_step, "npu", opt_local->npus);
	xfmt_tres(&job->ctx_params.tres_per_node, "npu",
	...
	xfmt_tres(&job->ctx_params.tres_per_socket, "npu",
		  opt_local->npus_per_socket);
	xfmt_tres(&job->ctx_params.tres_per_task, "npu",
