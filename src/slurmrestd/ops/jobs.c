					   ...
					   job_desc_msg_t *desc)
{
	...
	if (opt->cpus_per_npu)
		xstrfmtcat(desc->cpus_per_tres, "npu:%d", opt->cpus_per_npu);
	if (opt->npu_bind)
	...
	xfmt_tres_freq(&opt->tres_freq, "npu", opt->npu_freq);
	...
	xfmt_tres(&desc->tres_per_job, "npu", opt->npus);
	xfmt_tres(&desc->tres_per_node, "npu", opt->npus_per_node);
	...
	xfmt_tres(&desc->tres_per_socket, "npu", opt->npus_per_socket);
	xfmt_tres(&desc->tres_per_task, "npu", opt->npus_per_task);
