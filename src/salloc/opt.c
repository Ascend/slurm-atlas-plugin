...
static bool _opt_verify(void)
{
	...
	if (opt.npus_per_socket && (opt.sockets_per_node == NO_VAL)) {
		error("--npus-per-socket option requires --sockets-per-node specification");
		exit(error_exit);
	}
...
"  -U, --npus=n                count of NPUs required for the job\n"
